...
void ps2_init()
{
	install_IRQ_handler(1,  ps2_handler);

	// Clear buffer
	char __attribute__((__unused__)) scancode  = inb(0x60);
	

	kprintf("Initialized ps2 keyboard\n");
...
void ps2_handler(struct InterruptRegisters* regs)
{
    ...
    char press = inb(0x60) & 0x80; // Type of press
	...
	switch(scan_code)
	{
        ...
        default:
            if (press == 0){
                if (caps || caps_lock)
				{
                    kprintf("%c", uppercase[(uint32_t)scan_code]);
                }
				else
				{
                    kprintf("%c", lowercase[(uint32_t)scan_code]);
                }
            }
			break; 
    }
